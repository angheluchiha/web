...
if((isset($_POST["enviado"]))&& ($_POST["enviado"] =="form1")){
		...
		if(file_exists("img/user/".$_FILES["userfile"]["name"])){
...
<form class="form-horizontal" name="form1" id="form1" method="post" action="img_user.php" enctype="multipart/form-data">
...
<?php 
	...
	move_uploaded_file($_FILES["userfile"]["tmp_name"],"img/user/".$nombre_archivo);
	}else{
...
?>
<script>
opener.document.for_user.img_user.value="<?php echo $nombre_archivo; ?>";
opener.document.for_user.submit();
self.close();
</script>
